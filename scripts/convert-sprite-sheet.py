...
Output format (144x32 strip, 16x32 per frame):
  Standard order (default, matches swapped existing sprites):
    - 9 frames: [D-w1][D-idle][D-w2][U-w1][U-idle][U-w2][L-w1][L-idle][L-w2]
    - Walk1 is in position 0, drawn as "idle" by renderer (STRIP_COL_IDLE=0)
    - Use for all avatars except Metabee
  
  Metabee order (--metabee-order):
    - 9 frames: [D-idle][D-w1][D-w2][U-idle][U-w1][U-w2][L-idle][L-w1][L-w2]
    - Idle is in position 0, drawn as "idle" by renderer (STRIP_COL_IDLE=0)
    - Use only for Metabee
...
Examples:
  # Standard order (idle in frame 1)
  python3 scripts/convert-sprite-sheet.py originals/jarvis.png sprites/jarvis.png
...
def convert_sheet(input_path: str, output_path: str, side_facing: str = 'left', idle_col: int = 0, metabee_order: bool = False) -> None:
    ...
    # Default order: [walk1][idle][walk2] for each direction (standard order)
    # Metabee order: [idle][walk1][walk2] for each direction (legacy)
    if metabee_order:
        # Legacy: idle, walk1, walk2
        ...
        # Standard: walk1, idle, walk2 (frame 1 is idle)
...
def main():
    ...
    parser.add_argument('--metabee-order', action='store_true',
                        help='Use legacy frame order [idle][walk1][walk2] instead of standard [walk1][idle][walk2]')
